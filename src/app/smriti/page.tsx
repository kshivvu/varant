...
function MatraBadge({ score }: { score: number | null }) {
  ...
  return (
    <span
      className={`text-[11px] font-semibold px-2.5 py-1 rounded-full border ${color}`}
    >
      {score}% Matra
    </span>
  );
...
function MimamsaStatus({
  ...
  if (!isDue) {
    const daysLeft = Math.ceil(
      (new Date(entry.mimamsaDate).getTime() - Date.now()) / 86400000,
    );
    return (
      <p className="text-[11px] text-[#A0998F] mt-2">
        मीमांसा due in {daysLeft} days
      </p>
    );
  }
      ...
      {!editing ? (
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => {
                updateMimamsa(entry.id, text);
                onUpdate();
                setEditing(false);
              }}
              disabled={!text.trim()}
              className="text-[11px] font-bold uppercase tracking-[0.15em] bg-[#1A1510] text-white px-6 py-2.5 rounded-full disabled:opacity-40 hover:bg-[#9B1C1C] transition-all active:scale-95 shadow-md"
            >
              Record
            </button>
            <button
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
          ...
          {loaded && entries.length === 0 && (
            <div className="text-center py-24">
              ...
              <p className="text-[#1A1510] font-[family-name:var(--font-display)] text-2xl font-semibold mb-3">
                Your Smriti is empty
              </p>
              <p className="text-[#A0998F] text-sm mb-10 max-w-xs mx-auto leading-relaxed">
                Every Vichar you complete will be recorded here for your future
                Mimamsa.
              </p>
              <Link
                href="/demo"
                className="bg-[#1A1510] text-white px-8 py-4 rounded-full text-[11px] font-bold uppercase tracking-[0.15em] hover:bg-[#9B1C1C] transition-all shadow-lg active:scale-95 inline-block"
              >
          ...
          <div className="space-y-5">
            {entries.map((entry) => (
              <div
                key={entry.id}
                className="varant-council-card rounded-2xl p-6"
              >
                <div className="flex items-start justify-between gap-4 mb-3">
                  <p className="text-[16px] font-semibold text-[#1A1510] leading-snug flex-1">
                    &ldquo;{entry.question}&rdquo;
                  </p>
                  <MatraBadge score={entry.matraScore} />
                </div>
                {entry.recommendation && (
                  <p className="text-[13px] text-[#57534E] mb-3 line-clamp-2">
                    {entry.recommendation}
                  </p>
                )}
